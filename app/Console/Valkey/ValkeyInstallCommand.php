...
#[AsCommand(
    name: 'valkey:install',
    description: 'Install Valkey server on a server'
)]
class ValkeyInstallCommand extends BaseCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!($result['already_installed'] ?? false)) {
            $valkeyPass = $result['valkey_pass'] ?? null;

            if (null === $valkeyPass || '' === $valkeyPass) {
                $this->nay('Valkey installation completed but credentials were not returned');

                return Command::FAILURE;
            }

            /** @var string $valkeyPass */
            $this->yay('Valkey installation completed successfully');

            if ($displayCredentials) {
                $this->displayCredentialsOnScreen($valkeyPass);
            } else {
                /** @var string $saveCredentialsPath */
                try {
                    $this->saveCredentialsToFile($saveCredentialsPath, $server->name, $valkeyPass);
            ...
            $this->info('Valkey is already installed on this server');
        }
     ...
     */
    protected function displayCredentialsOnScreen(string $valkeyPass): void
    {
        $this->out([
            '',
            'Valkey Password:',
            ...
            '',
            'Connection string:',
            "  redis://:{$valkeyPass}@localhost:6379",
            '',
        ]);
    ...
    protected function saveCredentialsToFile(
        ...
        $content = <<<CREDS
            # Valkey Credentials for {$serverName}
            # Generated: {$this->now()}
            # WARNING: Keep this file secure!

            VALKEY_PASSWORD={$valkeyPass}

            ## Connection String (Valkey is Redis-compatible)
            REDIS_URL=redis://:{$valkeyPass}@localhost:6379
            CREDS;
